...
class LCNumero(QtWidgets.QWidget):
    def __init__(self, maxdigits):
        ...
        for x in range(maxdigits):
            ...
            import Code
            if Code.is_macos:
                lb.setAlignment(QtCore.Qt.AlignmentFlag(int(QtCore.Qt.AlignCenter)))
            else:
...
def svg2ico(svg, tam):
    ...
    if painter.begin(pm):
        ...
        # On macOS, painter.begin() may fail but we can still create the icon
        # Use a simpler approach for icon creation
        ico = QtGui.QIcon()
        ico.addPixmap(pm)
        return ico
    ico = QtGui.QIcon(pm)
...
def svg2pm(svg, tam):
    ...
    if painter.begin(pm):
        ...
        # On macOS, if painter.begin() fails, return empty pixmap to avoid errors
        pass
    return pm
...
def combine_pixmaps(pixmap1, pixmap2):
    ...
    if painter.begin(result):
        ...
        # On macOS, if painter fails, return empty pixmap to avoid errors
        pass
