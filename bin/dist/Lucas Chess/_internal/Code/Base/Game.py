...
def pgn_game(pgn):
    ...
    print(f"DEBUG Game.pgn_game li result: {li}")
    if li is None:
        print("DEBUG Game.pgn_game returning False - li is None")
        return False, game
    ...
    for elem in li:
        ...
        print(f"DEBUG Game.pgn_game processing elem: {repr(elem)}, key: {repr(key)}")
        if key == "[":
            kv = elem[1:].strip()
            pos = kv.find(" ")
            if pos > 0:
                ...
                vl = kv[pos + 1:].strip()
                lbup = lb.upper()
                print(f"DEBUG Game.pgn_game tag: {lb}={vl}")
                if lbup == "FEN":
                    ...
                    if clean_vl != FEN_INITIAL:
                        ...
                        si_fen = True
                        print(f"DEBUG Game.pgn_game FEN set successfully")
                elif lbup == "RESULT":
                    game.result = vl
            ...
            a1h8 = elem[1:]
            print(f"DEBUG Game.pgn_game processing move: {a1h8}")
            ...
            print(f"DEBUG Game.pgn_game move added successfully")
    ...
    if jg_activa:
        game.verify()
    print(f"DEBUG Game.pgn_game returning True, game with {len(game.li_moves)} moves")
    return True, game
